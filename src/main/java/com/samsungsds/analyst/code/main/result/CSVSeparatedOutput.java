...
public class CSVSeparatedOutput {
	private static final Logger LOGGER = LogManager.getLogger(CSVSeparatedOutput.class);

	private MeasuredResult result;

	public CSVSeparatedOutput(MeasuredResult result) {
		this.result = result;
	}
	...
	public void writeSonarIssue(List<SonarIssueResult> list) {
		String name = result.getLanguageType() == Language.JAVASCRIPT ? "sonarjs.csv" :
				(result.getIndividualMode().isSonarJava() && result.getIndividualMode().isJavascript() ? "sonarissue.csv" : "sonarjava.csv");
