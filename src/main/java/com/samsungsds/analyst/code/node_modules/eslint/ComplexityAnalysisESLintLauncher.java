...
import com.samsungsds.analyst.code.main.subject.FirstFileFinder;
...
public class ComplexityAnalysisESLintLauncher implements ComplexityAnalysis {
    ...
    public void run(String instanceKey) {
        ...
        String eslintDir = saveESLintAndGetPath(instanceKey);
        ...
        try {
            ...
            MeasuredResult.getInstance(instanceKey).addTempFileToBeDeleted(output);
            ...
            for (String srcPath : srcPaths) {
                ...
                FirstFileFinder find = new FirstFileFinder(path, "js");
                try {
                    find.getPath();
                } catch (IllegalStateException ise) {
                    LOGGER.info("src : {} (skip : there is no js file)", path);
                    continue;
                }
            ...
            if (!exists) {
                throw new IllegalStateException("There are no js files files in any directories : " + options.get("-dir"));
            }
    ...
    private String saveESLintAndGetPath(String instanceKey) {
        File zipFile = IOAndFileUtils.saveResourceFile("/statics/nodejs/ESLint/ESLint.zip", "eslint", ".zip");

        File dir = CacheUtils.getUnzippedCacheDirectory(instanceKey, zipFile, "eslint");
