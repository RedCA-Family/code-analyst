...
public abstract class GeneralLanguageCodeAnalystImpl extends AbstractCodeAnalystImpl {
	private static final Logger LOGGER = LogManager.getLogger(GeneralLanguageCodeAnalystImpl.class);

	private String outputFilePath;

	protected abstract Language getLanguageType();
	protected abstract String getSeparatedResultFileSuffix();
	protected abstract String getLanguageName();
	protected abstract boolean isBinaryArgumentNecessary();
	protected abstract boolean isInspectionCheck(AnalysisMode mode);
	protected abstract String getInspectionModeName();
	...
	public String analyze(String where, ArgumentInfo argument, AbstractFileInfo targetFile, boolean withSeparatedOutput) {
		...
		argument.getMode().setLanguageType(getLanguageType());
		...
		CliParser cli = new CliParser(arguments, getLanguageType());
	...
	public String analyzeWebResource(String where, WebArgumentInfo webArgument, WebTargetFileInfo webTargetFile, boolean includeCssAndHtml) {
		throw new UnsupportedOperationException("AnalyzeWebResource mode not supported in " + getLanguageName() + " language mode.");
	}
	...
	private String[] getArguments(String where, ArgumentInfo argument, AbstractFileInfo targetFile, boolean withSeperatedOutput) {
		...
		argumentList.add(getLanguageName().toLowerCase());
	...
	private String getModeParameter(AnalysisMode mode) {
		...
		if (isInspectionCheck(mode)) {
		    addAnalysisItem(parameter, getInspectionModeName());
        }
